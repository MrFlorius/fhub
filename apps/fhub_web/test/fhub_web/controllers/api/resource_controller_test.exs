defmodule FhubWeb.Api.ResourceControllerTest do
  ...
  describe "index" do
    test "lists all resources", %{conn: conn} do
      conn = get(conn, Routes.api_resource_path(conn, :index))
  ...
  describe "create resource" do
    test "renders resource when data is valid", %{conn: conn} do
      conn = post(conn, Routes.api_resource_path(conn, :create), resource: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.api_resource_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.api_resource_path(conn, :create), resource: @invalid_attrs)
  ...
  describe "update resource" do
    ...
    test "renders resource when data is valid", %{conn: conn, resource: %Resource{id: id} = resource} do
      conn = put(conn, Routes.api_resource_path(conn, :update, resource), resource: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, resource: resource} do
      conn = put(conn, Routes.api_resource_path(conn, :update, resource), resource: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete resource" do
    ...
    test "deletes chosen resource", %{conn: conn, resource: resource} do
      conn = delete(conn, Routes.api_resource_path(conn, :delete, resource))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.api_resource_path(conn, :show, resource))
      end
    end
  end
